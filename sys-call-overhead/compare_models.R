...
names(parameter_test) = c("model","layers","neurons","reps","threshold","algo","error function","activation function","mean nof_steps","avg. training duration / sec","nof nets","nof errors","mean nof steps","mean_rel_harmonic_error","best_rel_harmonic_error","mean_arith_error","mean_rel_arith_error","best_rel_arith_error","mean_rel_rms","best_rel_rms","mean_median_rel_error","mean_first_quartile_rel_error","mean_third_quartile_rel_error","max_max_rel_error","mean_arith_mean_test_rel_error","mean_weigthed_arith_mean_test_rel_error","mean_mean_in_range")
idx = 1
...
nof_iterations = 32 ### mindestens 2
for (nof_layers in 3:3) { ### Anzahl hidden-layers
  for (nof_neurons in 13:13) { ### Anzahl Neuronen pro hidden-layer
    ...
    for (nof_reps in 1:1) { ### Anzahl Wiederholungen des Trainingsprozesses
      for (threshold_exponent in 5:5) { ###  über 5 selten konvergent; über 4 Error
        for (threshold_summand in 0:9) { ### threshold des Algorithmus ist 10^(-threshold_exponent) + threshold_summand * 10^(-threshold_exponent);
            ...
            for (error_fct_idx in 1:1) { ### 1 - sse (sum of squared errors), 2 - ce (cross entropy)
              ...
              for (act_fct_idx in 1:1) { ### 1 - logistic, 2 - tanh
                ...
                for (model_idx in 1:1) { ###
                  ...
                  parameter_test[idx,]  = c(model,nof_layers,nof_neurons,nof_reps,test_threshold,algo,error_fct,act_fct,mean_steps,avg_duration,nof_iterations,error_count,values)
                  idx = idx + 1
                }
              }
            }
...
parameter_test$model[parameter_test$model == "single"] = 1
parameter_test$model[parameter_test$model == "double"] = 2
...
# write.table(parameter_test, "different_models", sep=",", row.names = FALSE)
