...
\begin{itemize}
	\item \textit{\textbf{MAE}} (vom englischen \textit{mean absolute error}) ist der arithmetische absolute Fehler gemittelt über alle Vorhersagen.
...
Für die Zuordnung in Fehlerklassen werden jedoch die Modellabweichungen eines anderen Modells benötigt. Die tatsächlichen Laufzeiten der Testdaten müssen für Modelle mit Fehlerklassen also bekannt sein.
\medskip
...
\textit{\textbf{LinRegFK Median agg}} kennt die Fehlerklassen, die aus der Clusteranalyse der Fehler von \textit{LinReg G} gewonnen worden, und \textit{\textbf{Tupel1FK Median agg}} die Klassen die aus den Modellabweichungen der besten Instanz von \textit{Tupel1} berechnet wurden.
\end{itemize}
...
\begin{figure}
	...
	\caption{Vergleich der Erstellung und Anwendung von NN-LinRegFK (oben) und NN-Tupel1FK(unten)}
...
\textit{%
	...
	Kapitel \ref{eval:benchmark} beschreibt im Detail, welche Messungen auf dem System durchgeführt wurden.
	Die erhaltenen Messdaten werden in Kapitel \ref{eval:exploration} genauer betrachtet. Die gewonnenen Informationen können bereits dabei helfen das E/A-System besser zu verstehen.
	Die erhaltenen Daten zu den Fehlerklassen werden in Kapitel \ref{eval:fk_analyse} genauer analysiert.
	Abschließend werden in Kapitel \ref{eval:leistungsvorhersage} die Leistungsvorhersagen der verschiedenen Modelle anhand der in Abschnitt \ref{analyse:valid} eingeführten Fehlermetriken und durch die Betrachtung der Verteilung der Modellabweichungen analysiert.
}
\bigskip
