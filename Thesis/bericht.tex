...
In dieser Bachelorarbeit wird das parallele Dateisystem eines Hochleistungsrechners analysiert und unter dem Einsatz künstlicher neuronaler Netze verschiedene Ansätze zur Modellierung der Ein-/Ausgabe-Leistung entwickelt und getestet.
Dabei erreichen die entwickelten künstliche neuronale Netze bei der Vorhersage von Zugriffszeiten geringere Modellabweichungen gegenüber den tatsächlichen Zugriffszeiten als lineare Modelle.\\
Es stellt sich heraus das der entscheidende Faktor für eine gute Modellierung des Ein-/Ausgabe-Systems darin liegt, zwischen gleichartigen Dateizugriffen, die allerdings zu verschiedenen Zugriffszeiten führen, zu unterscheiden.
Dazu können periodische Verhaltensmuster des Systems ausgenutzt werden. Diese gezielt für genauere Vorhersagen zu verwenden, erweist sich allerdings als schwierig.\\
Es zeigt sich, dass die Laufzeit eines Dateizugriffs wesentlich vom Verarbeitungspfad im System abhängt. Mit der Kenntnis dieses Pfades könnte die gewünschte Unterscheidung von Zugriffen mit gleichen Parametern erreicht werden.
Um eine Näherung der Verarbeitungspfade zu bestimmen, wird in dieser Bachelorarbeit ein Verfahren eingeführt, bei dem die Residuen eines Modells zur Erstellung von Klassen genutzt werden, welche wiederum den Verarbeitungspfaden entsprechen.\\
...
In Abbildung \ref{fig:densities_baselines_seq}c ist die mittlere vorhergesagten Laufzeit von \textit{LinRegFK Median agg} eingezeichnet.
...
Alternativ kann ein Ansatz wie die Zuhilfenahme von Fehlerklassen zur besseren Modellierung verwendet werden, dies ist aber natürlich \glqq unfair\grqq{}, da in den Klassen Informationen über die tatsächliche Laufzeit der E/A-Aufrufe stecken.
 ...
 \begin{figure}
 	\centering
 	\subfloat[Modell \textit{Durchschnitt} angewendet auf SEQ. Mitt.-Fehler = 20.77\%, Zugriffsgröße = Delta-Abstand = 512\,KiB, OpTyp = W]{
 		\includegraphics[width=.43\textwidth]{Bilder/Plots/baselines/Dichten/plot_density_best_mean_performance.png}
 	}\hfill
 	\subfloat[Modell \textit{LinReg G} angewendet auf SEQ. Mitt.-Fehler = 6.74\%, Zugriffsgröße = Delta-Abstand: 64\,KiB, OpTyp: W]{
 		\includegraphics[width=.43\textwidth]{Bilder/Plots/baselines/Dichten/plot_density_best_linreg_Size.png}
 	}\\
 	\subfloat[Modell \textit{LinRegFK Median agg} angewendet auf SEQ. Mitt. - Fehler = 2.6\%, Zugriffsgröße = Delta-Abstand = 8\,MiB, OpTyp = W]{
 	...
 	\subfloat[Modell \textit{Median agg} angewendet auf SEQ. Mitt. - Fehler = 68.65\%, Zugriffsgröße = Delta-Abstand = 512\,KiB, OpTyp = W]{
...
Die Abhängigkeit der Laufzeit von Delta-Abstand scheint jedoch weiterhin sehr gering, sodass sich die Varianz der Vorhersagen in Grenzen hält.
...
Bei der Analyse dieser Messdaten ist die starke lineare Korrelation zwischen Zugriffsgröße und Laufzeit entdeckt worden, zusätzlich konnte gezeigt werden, dass tatsächlich periodisches Verhalten in den Zugriffszeiten auftritt, dieses könnte entsprechend zur Bestimmung der E/A-Pfade genutzt werden.
...
Zum einen könnten andere Modelle zum Erstellen der Fehlerklassen untersucht werden und zum anderen müsste der Zusammenhang zwischen E/A-Pfad und Fehlerklassen genauer analysiert werden. So könnte man versuchen, die Laufzeiten der Zugriffe in einer Fehlerklasse mit ihrer hauptsächlichen Ursache zu korrelieren.
