...
Maschinelles Lernen überträgt Aufgaben aus einem Gebiet auf den Computer in dem Menschen sehr fortgeschritten sind, daher gehört es auch zum Feld der künstlichen Intelligenz. Verfahren dieser Disziplin versuchen durch intelligentes lernen von Mustern Vorhersagen und Entscheidungen, wie die Zuordnung zu einer bestimmten Klasse, zu treffen. Intelligent bedeutet hierbei (beispielweise), dass vorgegebene Informationen nicht schlicht auswendig gelernt und wiedergegeben werden, sondern von diesen Informationen abstrahiert wird, um Gesetzmäßigkeiten innerhalb der Trainingsdaten zu erkennen.
Die Trainingsdaten sind die Informationen, die dem Algorithmus bekannt sind, von diesen versucht er zu lernen. Mit einem Testdatensatz kann anschließend die Ausgabe zu bisher unbekannten Daten bewertet werden.
Zur Anwendung eines Verfahrens des maschinellen Lernens müssen Kriterien zunächst Kriterien bzw. Leistungsmetriken eingeführt werden, anhand derer die Qualität der Vorhersagen und Entscheidungen gemessen werden kann. Einerseits zum Vergleich verschiedener Ansätze, aber insbesondere für den Lernprozess des Algorithmus, damit dieser sozusagen aus seinen Fehlern und Erfolgen lernen kann. Einfache Metriken wären beispielsweise für einen Klassifizierungsalgorithmus der Anteil falsch klassifizierter Datenpunkte.
...
\cite{Alpaydin:2010:IML:1734076}. 
...
Für MLPs hat Cybenko \cite{cybenko:mcss} bewiesen, dass sie belieblige kontinuierliche Funktionen auf einer kompakten Teilmenge des euklidischen Raums $\mathbb{R}^n$ approximieren kann. Dies wird auch als "universal approximation theorem" bezeichnet.
