...
\end{itemize}
	...
	Für den selben Zweck sind \textit{RQ3} und \textit{RMax} nützlich.
\begin{itemize}
...
Regelmäßigere Ausreißer sind ein Zeichen für systematisch anders behandelte E/A-Zugriffe, die sich vielleicht durch verschiedene E/A-Pfade unterscheiden.
...
Eine Ausnahme bilden die Fehlerklassen-Modelle, die Modelle können weiterhin sehr gute mittlere Vorhersagen treffen, einige Messungen können auf RND allerdings scheinbar sehr schlecht vorhergesagt werden, sodass \textit{RMax} und \textit{MSPE} höher als bei sequentiellen Datensatz sind.\medskip
...
Alternativ kann ein Ansatz, wie die Zuhilfenahme von Fehlerklassen, zur besseren Modellierung verwendet werden, dies ist aber natürlich \glqq unfair\grqq{}, da in den Klassen Informationen über die tatsächliche Laufzeit der E/A-Aufrufe stecken.
 ...
 \begin{figure}
 	\centering
 	\subfloat[Modell \textit{Durchschnitt} angewendet auf SEQ. Mitt.-Fehler = 20.77\%, Zugriffsgröße = Delta-Abstand = 512KiB, OpTyp = W]{
 		\includegraphics[width=.43\textwidth]{Bilder/Plots/baselines/Dichten/plot_density_best_mean_performance.png}
 	}\hfill
 	\subfloat[Modell \textit{LinReg G} angewendet auf SEQ. Mitt.-Fehler = 6.74\%, Zugriffsgröße = Delta-Abstand: 64KiB, OpTyp: W]{
 		\includegraphics[width=.43\textwidth]{Bilder/Plots/baselines/Dichten/plot_density_best_linreg_Size.png}
 	}\\
 	\subfloat[Modell \textit{LinRegFK Median agg} angewendet auf SEQ. Mitt. - Fehler = 2.6\%, Zugriffsgröße = Delta-Abstand = 8MiB, OpTyp = W]{
 	...
 	\subfloat[Modell \textit{Median agg} angewendet auf SEQ. Mitt. - Fehler = 68.65\%, Zugriffsgröße = Delta-Abstand = 512KiB, OpTyp = W]{
 		\includegraphics[width=.43\textwidth]{Bilder/Plots/baselines/Dichten/plot_density_worst_median_Duration_aggregated.png}
 	} 
 	\caption{Laufzeit-Dichte einiger Referenzodelle auf SEQ; 90\% der Werte sind größer als die grüne, 10\% sind größer als die pinke Linie, die mittlere Dauer entspricht der schwarzen Linie, die blaue Linie ist die mittlere Vorhersage des Modells zu diesen Attributen}
...
\begin{table}
	...
	\makebox[\textwidth][c]{
		\subfloat[Fehlermetriken auf SEQ]{
			\begin{tabular}{|r|r|r|r|r|r|r|r|r|}\hline%
				Modell & MAE (s) & Avg-MAPE (\%) & Train-MAPE (\%) &  MAPE (\%) & MSPE (\%) & RQ3 (\%) & RMax (\%) & In-range (\%) \\\hline\hline
				\csvreader[late after line=\\\hline]%
...
Während unter den Netzen zu diesem Modell, das mit dem geringsten \textit{MSPE} einen \textit{MAPE} von nur 24\% erreicht, beträgt der durchschnittliche \textit{MAPE} über die 12 Netze über 37\,000 mal der tatsächlichen Laufzeit.
...
Wie der sehr hohe \textit{RMax}-Wert von 132820\% und der \textit{MSPE} mit 2124\% gegenüber dem \textit{MAPE} mit 234\% andeuten, scheint das Modell einige Messungen äußerst schlecht vorhersagen zu können.
...
\begin{figure}
	\centering
	\subfloat[Vorhersage der Laufzeiten]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_onlyPred_tuple1_with_error_class_from_linreg_Duration_seq.png}
	}
	\hfill
	\subfloat[Näherungen des Modells zu den Quantilen der Laufzeiten, Quantil 0.1 in grün und Quantil 0.9 in rot]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_tuple1_with_error_class_from_linreg_Duration_seq.png}
	}\\
	\subfloat[Die am schlechtesten vorhergesagten Laufzeiten sind in rot markiert]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_biggest1_errors_tuple1_with_error_class_from_linreg_Duration_seq.png}
	}
	\hfill
	\subfloat[Die am besten vorhergesagten Laufzeiten sind in grün markiert]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_smallest1_errors_tuple1_with_error_class_from_linreg_Duration_seq.png}
	}
...
\begin{figure}
	\centering
	\subfloat[Vorhersage der Laufzeiten]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_onlyPred_tuple1_with_error_class_from_linreg_Duration_rnd.png}
	}\\
	\subfloat[Die am schlechtesten vorhergesagten Laufzeiten sind in rot markiert]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_biggest1_errors_tuple1_with_error_class_from_linreg_Duration_rnd.png}
	}
	\hfill
	\subfloat[Die am besten vorhergesagten Laufzeiten sind in grün markiert]{
		\includegraphics[width=.43\textwidth]{Bilder/Plots/models/plot_smallest1_errors_tuple1_with_error_class_from_linreg_Duration_rnd.png}
	}
...
Zunächst wurde im Hintergrund-Kapitel (Kapitel \ref{Hintergrund}) erläutert, was Ein-/Ausgabe ist und wie sie grundlegend funktioniert, es wurde aufgezeigt, was einen Hochleistungsrechner auszeichnet und weshalb Leistungsvorhersage zu einem solchen System besondere Schwierigkeiten mit sich bringt, und zuletzt wurde die Funktionsweise von neuronalen Netzen im Kontext des maschinellen Lernens dargelegt.\\
Im folgenden Kapitel \ref{verwandte_arbeiten} über verwandte Arbeiten wurden zunächst die zwei verschiedenen Ansätze bei der E/A-Leistungsvorhersage eingeführt. So muss zwischen White-Box-Modellierung, bei der Hardware-Details beachtet werden, und der Black-Box-Modellierung, bei der ein abstraktes Modell aus einer Regressionsanalyse erstellt wird, unterschieden werden. Desweiteren wurden Veröffentlichungen zur Leistungsvorhersage im Kontext von neuronalen Netzen und Hochleistungsrechnen präsentiert.\\
...
Nachdem darauf eingegangen wurde, wie Modelle des E/A-Systems in dieser Arbeit validiert werden, wurden die verschiedenen Modellklassen und anschließend die untersuchten Modelle selbst vorgestellt. Unterschieden wird zwischen Referenzmodellen, bei denen es sich hauptsächlich um Kontrollmodelle handelt, NN-Modellen, die neuronale Netze nutzen, und Fehlerklassenmodellen, mit denen die Auswirkung der Kenntnis von E/A-Pfaden auf die Laufzeitvorhersagen überprüft werden kann.
...
Zunächst wurden die durchgeführten Benchmark-Tests erläutert, diese bestehen aus einer Vielzahl Messreihen, die zwei Anwendungstypen darstellen. Zum einen den sequentiellen Zugriff auf hintereinanderliegenden Daten und zum anderen Zugriffe an unvorhersehbaren Stellen der Datei. Diese Messdaten werden für die NN-Modelle in Trainings- und Testdaten aufgeteilt.
Bei der Analyse dieser Messdaten ist die starke lineare Korrelation zwischen Zugriffsgröße und Laufzeit entdeckt wurden, zusätzlich konnte gezeigt werden, dass tatsächlich periodisches Verhalten in den Zugriffszeiten auftritt, die entsprechend zur Bestimmung der E/A-Pfade genutzt werden könnten.
...
Mit Hilfe der Fehlerklassen können die mittleren Fehler (ansonsten vergleichbarer Modelle) hingegen auf 0.02 Millisekunden zu den Messungen mit sequentiellen Zugriff und etwa 0.1 Millisekunden zu den Messungen mit zufälligen Dateizugriff gebracht werden.
...
\subsubsection{Zukünftige Arbeit}
...
Crume und Maltzahn haben mit ihrem Ansatz Fourier-Analyse zur Untersuchung dieser periodischen Zusammenhänge auf einzelnen Festplatten zu nutzen Erfolge aufzeigen können \cite{Crume:2013:FML:2538542.2538561}.
...
Zum einen könnten andere Modelle zum Erstellen der Fehlerklassen untersucht werden und zum anderen müsste der Zusammenhang zwischen E/A-Pfad und Fehlerklassen genauer analysiert werden. So könnte man versuchen die Laufzeiten der Zugriffe in einer Fehlerklasse mit 
ihrer hauptsächlichen Ursache zu korrelieren.
